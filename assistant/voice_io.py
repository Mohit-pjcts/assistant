"""Voice I/O building blocks: mic capture, local STT (faster-whisper), TTS
...
STT runs `faster-whisper` locally (verified installing and transcribing on
this project's 3.12 arm64 venv — STEPS.md 37/38): zero per-utterance API
cost, no network dependency. TTS uses macOS `say`, invoked argv-only
(`shell=False`) for consistency with the rest of the codebase's subprocess
posture, though — unlike mac_tools.py's allowlisted actions — this isn't a
gated agent-invocable tool: it's the voice harness's own output rendering,
equivalent to main.py's `print()`, never something the model chooses to
call. The voice is configurable (ASSISTANT_TTS_VOICE env var) and checked
against the actually-installed voices once, falling back to the system
default with a logged warning — a not-yet-downloaded Enhanced/Premium voice
...
def preload_stt_model() -> None:
    """Load the STT model ahead of first use — the first load downloads and
    initializes the model (several seconds), which would otherwise land as
...
def transcribe(audio: np.ndarray) -> str:
    ...
    if audio.size == 0:
        return ""
    segments, _ = _get_stt_model().transcribe(audio, language="en")
